...
- Household organization that gives every resident, personal collection, and material pastime a storage demand; derives capacity from bookcases, desks, tables, and kitchen storage; combines it with clear floor area; and feeds visible clutter pressure back into home quality
...
- Persistent player-authored room names with validation, Undo and Redo, save migration, and consistent wayfinding across Home Simulator selection, circulation and safety evidence, resident room claims, and City Explorer interior location readouts
...
- One-click furnishing duplication that searches outward for the nearest legal position, preserves style, variant, tint, rotation, and floor, clears resident ownership, charges the exact catalog price, selects the copy, and reverts as one Undo-safe edit
- Furnishing design eyedropper that samples style, design variant, and optional custom tint from any selected object, visibly retains the sample across catalog choices, applies it inside the original placement command, and supports an explicit reset
- Shift-click furnishing multi-select with visible group highlighting, shared style, variant, tint, rotation, collision-safe relative movement, exact combined sale refunds, and one atomic Undo checkpoint per group operation
- Live furnishing placement feedback with green or red footprint previews, complete group footprints during relative movement, floor and exact-price context, budget warnings, and collision or room-boundary explanations before committing a click
- Live room-drawing previews with the complete green or red footprint, dimensions, area, exact shell price, floor, budget state, parcel-boundary enforcement, and same-floor overlap rejection before the second corner is committed
- Unified move-in readiness that weighs safety, room circulation, household capacity, storage, energy, bedroom privacy, and condition; distinguishes unsafe homes from nearly ready and ready plans; and presents severity-ranked blockers in the build toolbar and parcel inspector
- Persistent household move-in approval that refuses unsafe or capacity-blocked homes, records the approved time and readiness score, remains visible in home and parcel controls, and automatically suspends whenever the live home stops meeting readiness requirements
- Persistent move-in goals generated from the three highest live readiness priorities, displayed in Home Simulator and parcel evidence, and completed automatically from actual safety, capacity, organization, energy, privacy, or condition improvements
- One-time first-night simulation gated by active move-in approval, with residents placed into their claimed rooms, physical furnishing-based activities, immediate comfort and stress effects, permanent homecoming milestones, and persistent household completion evidence
...
- Visible inspection trends with improving, declining, steady, or insufficient-history states, signed score change, best score, record count, and failed-to-passed recognition in Home Simulator and parcel evidence
